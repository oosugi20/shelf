...
$(function(){
	$('#localnavi dd.menu').mk_toggleMenu({
		children: '.menu-lv2'
	});
});
...
$.fn.mk_toggleMenu = function(config){
	...
	this.each(function(){
					...
					.click(function(){
						if($(this).html()==closeButton){
							...
							$(this).parent()
								...
								.addClass(closeClass)
								.find(children).stop(false,true).slideUp(upSpeed);
							...
							$(this).parent()
								...
								.find(children).stop(false,true).slideDown(downSpeed);
						}
					});
		};
		...
		// begiing
		$(this).addClass(closeClass).find(children).slideUp(0);
